...
if __name__ == '__main__':
    ...
    #cap = cv.VideoCapture('cam/MAH03119.MP4')
    cap = cv.VideoCapture('100_1107/IMGP0012.AVI')
    ...
    while(1):
	    ...
	    print(info.dim_num)
	    print(info.size[0], info.size[1], info.size[2], info.size[3])
	    	...
	    	for box, score, cl in zip(boxes, scores, classes):
	    		...
	    		if cl == 1:
	    			...
	    			if boxes2 is not None:
	    				...
	    				if len(detection) == 6 and detection != plate_number and ok_plate == False:
	    					plate_number = detection
	    					plate_acc = final_accuracy
	    					encoded1,buffer1 = cv.imencode('.jpg',imgg_for_database,[cv.IMWRITE_JPEG_QUALITY,15])
	    					b64_plate = base64.b64encode(buffer1).decode("utf-8")
	    					ok_plate = True
	    			...
	    			if cv.waitKey(1) & 0xFF == ord('q'):
	    				break
	    		if cl == 0 and ok_car == False:
	    			part2 = imggg[max(int(y),0):min(int(y+h),720) , max(int(x),0):min(int(x+w), 1280)]
	    			encoded,buffer = cv.imencode('.jpg',part2,[cv.IMWRITE_JPEG_QUALITY,15])
	    			b64_car = base64.b64encode(buffer).decode("utf-8")
	    			ok_car = True
	    			
	    		if ok_car and ok_plate:
	    		
	    			cv.imwrite("cars_detections/" + nowstr + ".jpg", part2)
	    			cars_collection.insert_one({'filename' : nowstr, 'data' : b64_car});
	    			
	    			
	    			cv.imwrite("plates_detections/" + nowstr + ".jpg", imgg_for_database)
	    			images_collection.insert_one({'filename' : nowstr, 'data' : b64_plate})
	    ...
	    cv.putText(img, plate_number, (50, 50), cv.FONT_HERSHEY_SIMPLEX, 2, (255, 0, 255), 3)
	    
	    encoded,buffer = cv.imencode('.jpg',img,[cv.IMWRITE_JPEG_QUALITY,20])
